...
class Tournament():
    ...
    def swissPairings(self):
        ...
        cur.execute('''SELECT player1.id as id1, 
                        ...
                        FROM players as player1, players as player2, player_registration as pr
                        WHERE player1.id < player2.id
                        AND (SELECT wins FROM standings WHERE player_id = player1.id) =
                            (SELECT wins FROM standings WHERE player_id = player2.id)
                        AND player1.id = pr.id
                        AND player2.id = pr.id
                        AND pr.tournament_id = (%s)''', (self.id,))
